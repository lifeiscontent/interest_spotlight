defmodule InterestSpotlightWeb.AdminAuth do
  ...
  alias InterestSpotlight.Backoffice.Scope

  # Make the remember me cookie valid for 14 days. This should match
  # the session validity setting in AdminToken.
  @max_cookie_age_in_days 14
  @remember_me_cookie "_interest_spotlight_web_admin_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_admin(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      InterestSpotlightWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_scope_for_admin(conn, _opts) do
    with {token, conn} <- ensure_admin_token(conn),
         {admin, token_inserted_at} <- Backoffice.get_admin_by_session_token(token) do
      conn
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      InterestSpotlightWeb.Endpoint.broadcast(admin_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule InterestSpotlightWeb.PageLive do
        use InterestSpotlightWeb, :live_view

        on_mount {InterestSpotlightWeb.AdminAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{InterestSpotlightWeb.AdminAuth, :require_authenticated}] do
  ...
  def on_mount(:require_sudo_mode, _params, session, socket) do
    ...
    if Backoffice.sudo_mode?(socket.assigns.current_scope.admin, -10) do
      {:cont, socket}
    else
      socket =
        socket
