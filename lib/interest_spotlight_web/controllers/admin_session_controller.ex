defmodule InterestSpotlightWeb.AdminSessionController do
  ...
  defp create(conn, %{"admin" => %{"token" => token} = admin_params}, info) do
    case Backoffice.login_admin_by_magic_link(token) do
  ...
  def update_password(conn, %{"admin" => admin_params} = params) do
    ...
    {:ok, {_admin, expired_tokens}} = Backoffice.update_admin_password(admin, admin_params)
