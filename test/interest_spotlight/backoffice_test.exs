defmodule InterestSpotlight.BackofficeTest do
  use InterestSpotlight.DataCase

  alias InterestSpotlight.Backoffice

  import InterestSpotlight.BackofficeFixtures
  alias InterestSpotlight.Backoffice.{Admin, AdminToken}
  ...
  describe "register_admin/1" do
    ...
    test "validates email when given" do
      {:error, changeset} = Backoffice.register_admin(%{email: "not valid"})
    ...
    test "validates maximum values for email for security" do
      ...
      {:error, changeset} = Backoffice.register_admin(%{email: too_long})
  ...
  describe "sudo_mode?/2" do
    test "validates the authenticated_at time" do
      ...
      assert Backoffice.sudo_mode?(%Admin{authenticated_at: DateTime.utc_now()})
      assert Backoffice.sudo_mode?(%Admin{authenticated_at: DateTime.add(now, -19, :minute)})
      refute Backoffice.sudo_mode?(%Admin{authenticated_at: DateTime.add(now, -21, :minute)})

      # minute override
      refute Backoffice.sudo_mode?(
               %Admin{authenticated_at: DateTime.add(now, -11, :minute)},
               -10
             )

      # not authenticated
      refute Backoffice.sudo_mode?(%Admin{})
    end
  end
  ...
  describe "update_admin_email/2" do
    ...
    test "updates the email with a valid token", %{admin: admin, token: token, email: email} do
      assert {:ok, %{email: ^email}} = Backoffice.update_admin_email(admin, token)
    ...
    test "does not update email with invalid token", %{admin: admin} do
      assert Backoffice.update_admin_email(admin, "oops") ==
    ...
    test "does not update email if token expired", %{admin: admin, token: token} do
      ...
      assert Backoffice.update_admin_email(admin, token) ==
               {:error, :transaction_aborted}
  ...
  describe "change_admin_password/3" do
    ...
    test "allows fields to be set" do
      changeset =
        Backoffice.change_admin_password(
          %Admin{},
          %{
  ...
  describe "update_admin_password/2" do
    ...
    test "validates password", %{admin: admin} do
      {:error, changeset} =
        Backoffice.update_admin_password(admin, %{
          password: "not valid",
    ...
    test "validates maximum values for password for security", %{admin: admin} do
      ...
      {:error, changeset} =
        Backoffice.update_admin_password(admin, %{password: too_long})
  ...
  describe "get_admin_by_session_token/1" do
    ...
    test "returns admin by token", %{admin: admin, token: token} do
      assert {session_admin, token_inserted_at} = Backoffice.get_admin_by_session_token(token)
  ...
  describe "get_admin_by_magic_link_token/1" do
    ...
    test "returns admin by token", %{admin: admin, token: token} do
      assert session_admin = Backoffice.get_admin_by_magic_link_token(token)
      assert session_admin.id == admin.id
    end

    test "does not return admin for invalid token" do
      refute Backoffice.get_admin_by_magic_link_token("oops")
    end

    test "does not return admin for expired token", %{token: token} do
      {1, nil} = Repo.update_all(AdminToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      refute Backoffice.get_admin_by_magic_link_token(token)
    end
  end
  ...
  describe "login_admin_by_magic_link/1" do
    test "confirms admin and expires tokens" do
      ...
      assert {:ok, {admin, [%{token: ^hashed_token}]}} =
               Backoffice.login_admin_by_magic_link(encoded_token)
    ...
    test "returns admin and (deleted) token for confirmed admin" do
      ...
      assert admin.confirmed_at
      {encoded_token, _hashed_token} = generate_admin_magic_link_token(admin)
      assert {:ok, {^admin, []}} = Backoffice.login_admin_by_magic_link(encoded_token)
      # one time use only
      assert {:error, :not_found} = Backoffice.login_admin_by_magic_link(encoded_token)
    end
  ...
  describe "deliver_login_instructions/2" do
    ...
    test "sends token through notification", %{admin: admin} do
      token =
        extract_admin_token(fn url ->
          Backoffice.deliver_login_instructions(admin, url)
        end)
