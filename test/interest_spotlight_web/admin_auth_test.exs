defmodule InterestSpotlightWeb.AdminAuthTest do
  ...
  alias InterestSpotlightWeb.AdminAuth
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, InterestSpotlightWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_admin/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      InterestSpotlightWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "fetch_current_scope_for_admin/2" do
    ...
    test "reissues a new token after a few days and refreshes cookie", %{conn: conn, admin: admin} do
      ...
      {admin, _} = Backoffice.get_admin_by_session_token(token)
  ...
  describe "on_mount :require_sudo_mode" do
    ...
    test "redirects when authentication is too old", %{conn: conn, admin: admin} do
      ...
      admin_token = Backoffice.generate_admin_session_token(admin)
      {admin, token_inserted_at} = Backoffice.get_admin_by_session_token(admin_token)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        InterestSpotlightWeb.Endpoint.subscribe("admins_sessions:#{Base.url_encode64(token)}")
      end
